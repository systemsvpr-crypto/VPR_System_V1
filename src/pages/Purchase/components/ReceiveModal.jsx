...
import {
  createDelivery,
  updateDelivery,
  generateNextLiftingNumber,
  getDeliveriesForItem,
...
const QtyCard = ({ label, value, color, icon: Icon, sub }) => {
  ...
  return (
    <div
      className={`flex flex-col items-center justify-center rounded-xl border px-3 py-2 gap-0.5 ${c.bg} ${c.border}`}
    >
...
const ReceiveModal = ({
  ...
  const defaultTabStatus = tabMode === "aawak-details" || tabMode === "in-transport-godown" ? "In Transport Godown" : "In Transit";
  ...
  const [form, setForm] = useState({
    ...
    vehicle_number: "",
    remarks: "",
    status: initialStatus || defaultTabStatus,
  });
  ...
  const statusOptions = useMemo(() => {
    let options = [];
    if (tabMode === "in-transit") {
      options = [{ id: "In Transit", label: "In Transit" }];
    } else if (tabMode === "aawak-details" || tabMode === "in-transport-godown") {
      options = [
    ...
    if (editingLift && editingLift.status && !options.some((o) => o.id === editingLift.status)) {
      const labels = {
        "In Transit": "In Transit",
        "In Transport Godown": "In Transport Godown",
        Arrived: "Arrived",
      };
      options = [
        { id: editingLift.status, label: labels[editingLift.status] || editingLift.status },
        ...options,
      ];
    }
    return options;
  ...
  const handleTransporterChange = (transporterId) => {
    const selectedTransporter = (transporters || []).find(
      (t) => String(t.transporter_id) === String(transporterId)
    );

    const lastDeliveryForTransporter = (deliveryHistory || []).find(
      (d) => String(d.transporter_id) === String(transporterId)
    );

    const autoVehicle =
      selectedTransporter?.vehicle_number ||
      selectedTransporter?.vehicle_no ||
      lastDeliveryForTransporter?.vehicle_number ||
      "";

    const autoLr =
      selectedTransporter?.lr_number ||
      selectedTransporter?.lr_no ||
      lastDeliveryForTransporter?.lr_number ||
      "";

    setForm((prev) => ({
      ...prev,
      transporter_id: transporterId,
      vehicle_number: autoVehicle || prev.vehicle_number,
      lr_number: autoLr || prev.lr_number,
    }));
  };
  ...
  const handleEditLift = (lift) => {
    ...
    setForm({
      ...
      expected_delivery_date: lift.expected_delivery_date
        ? lift.expected_delivery_date.split("T")[0]
        : "",
      ...
      status: lift.status || initialStatus,
    });
  };

  const handleCancelEdit = () => {
    setEditingLift(null);
    const defaultGodownId =
      item.approved_godown_id || item.purchase_indents?.godown_id || "";
    setForm({
      delivery_date: getTodayLocalString(),
      expected_delivery_date: "",
      ...
      getDeliveriesForItem(item.item_id)
        .then((res) => {
          ...
          const totalAllocated = list.reduce(
            (sum, d) => sum + Number(d.received_quantity || 0),
            0,
          );
  ...
  const godownOptions = activeGodowns.map((g) => ({
    value: g.godown_id,
    label: g.name,
  }));
  ...
  const maxAllowedQty = useMemo(() => {
    ...
    if (editingLift) {
      const otherAllocated =
        totalAllocatedQty - Number(editingLift.received_quantity || 0);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.received_quantity || Number(form.received_quantity) <= 0) {
      toast.error("Please enter a valid received quantity.");
      return;
    }
    if (exceedRemaining) {
      toast.error(
        `Quantity (${formQty}) exceeds allowed quantity (${maxAllowedQty}).`,
      );
      return;
    }
    ...
    if (form.status === "Arrived" && form.delivery_date.slice(0, 10) > today) {
      toast.error(
        "Lifting date cannot be a future date when marking as Arrived.",
      );
      return;
    }
                ...
                <div className="rounded-xl border border-slate-200 overflow-hidden">
                  <div className="flex items-center justify-between px-3 py-2 border-b border-slate-200 bg-slate-50">
                    <span className="flex items-center gap-1.5 text-xs font-semibold text-slate-700">
                      ...
                      {editingLift
                        ? `Edit Lift — ${editingLift.lifting_number}`
                        : maxAllowedQty <= 0
                          ? "New Lift"
                          : `New Lift — ${nextLifting || "Generating..."}`}
                    </span>
                    {editingLift && (
                      <button
                        type="button"
                  ...
                  {maxAllowedQty <= 0 && !editingLift ? (
                    <div className="px-3 py-6 text-center text-xs text-slate-400 bg-slate-50/50 rounded-lg border border-dashed border-slate-200 m-3">
                      ...
                      <p>
                        All {orderedQty} unit(s) have been fully allocated to
                        lifts.
                      </p>
                    </div>
                  ) : (
                    <div className="px-3 py-3 grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setForm({
                              ...form,
                              received_quantity: e.target.value.replace(
                                /\D/g,
                                "",
                              ),
                            })
                        ...
                        {exceedRemaining && (
                          <p className="text-xs text-red-500 mt-1">
                            Exceeds allowed quantity ({maxAllowedQty})
                          </p>
                        )}
                      </div>
                      <div>
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <DatePicker
                          ...
                          onChange={(val) =>
                            setForm({
                              ...form,
                              expected_delivery_date: val.target
                                ? val.target.value
                                : val,
                            })
                          }
                      ...
                      <div className="sm:col-span-2">
                        ...
                        <div className="flex items-center gap-1 bg-slate-100 rounded-lg p-0.5 w-fit">
                          {statusOptions.map((s) => {
                            const style = STATUS_STYLES[s.id] || STATUS_STYLES["In Transit"];
                              ...
                              <button
                                ...
                                onClick={() => {
                                  ...
                                  if (s.id === "Arrived") {
                                    ...
                                    if (
                                      form.delivery_date &&
                                      form.delivery_date.slice(0, 10) > today
                                    ) {
                        ...
                        {statusOptions.some((o) => o.id === "Arrived") && (
                          <p className="text-[10px] text-slate-400 mt-1">
                            Stock will only be updated when status is set to{" "}
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) =>
                            setForm({ ...form, lr_number: e.target.value })
                          }
                        />
                      </div>
                      <div>
              ...
              <div className="rounded-xl border border-slate-200 overflow-hidden self-start">
                <div className="flex items-center justify-between px-3 py-2 bg-slate-50 border-b border-slate-200">
                  ...
                  {deliveryHistory.length > 0 && (
                    <span className="text-[10px] font-medium text-slate-400">
                      {deliveryHistory.length} lift
                      {deliveryHistory.length !== 1 ? "s" : ""}
                    </span>
                  )}
                </div>
                ...
                {loadingHistory && deliveryHistory.length === 0 ? (
                  ...
                  <div className="px-3 py-6 text-center">
                    <ClipboardList
                      size={20}
                      className="text-slate-200 mx-auto mb-1.5"
                    />
                    ...
                    <table className="w-full text-[11px]">
                      <thead>
                        <tr className="border-b border-slate-200 bg-slate-50/60">
                          <th className="text-left px-2.5 py-1.5 font-semibold text-slate-500 uppercase tracking-wide whitespace-nowrap">
                            Lift No.
                          </th>
                          <th className="text-left px-2.5 py-1.5 font-semibold text-slate-500 uppercase tracking-wide">
                            Date
                          </th>
                          <th className="text-center px-2.5 py-1.5 font-semibold text-slate-500 uppercase tracking-wide">
                            Qty
                          </th>
                          <th className="text-left px-2.5 py-1.5 font-semibold text-slate-500 uppercase tracking-wide">
                            Godown
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {deliveryHistory.map((del) => (
                          ...
                          >
                              ...
                              {del.purchase_delivery_godowns?.length > 0 ? (
                                <span className="flex flex-wrap gap-0.5">
                                  {del.purchase_delivery_godowns.map((g) => (
                                    <span
                                      key={g.godown_id}
                                      className="inline-flex items-center gap-0.5 px-1 py-0.5 rounded bg-white border border-slate-200 text-[10px]"
                                    >
                                      {g.godowns?.name || "—"}
                                    </span>
                                  ))}
                            ...
                            <td
                              className="px-2.5 py-2 text-slate-600 max-w-[100px] truncate"
                              title={del.transporters?.name || ""}
                            >
                            ...
                            <td className="px-2.5 py-2 text-center">
                              {(() => {
                                const style =
                                  STATUS_STYLES[del.status || "Received"] ||
                                  STATUS_STYLES["In Transit"];
                                const SIcon = style.icon;
                                return (
                                  <span
                                    className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-medium border ${style.bg} ${style.text} ${style.border}`}
                                  >
                                    <SIcon size={10} />{" "}
                                    {del.status || "Received"}
                                  </span>
